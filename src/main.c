...
#define F_SAMPLING      (F_CPU/(PRESCALER*510))     // 510 comes from timer's dual slope operation, 0->255->1
#define F_WAVE          60
#define PHASE_OFFSET    (F_WAVE*(1ULL<<32)/F_SAMPLING)

volatile uint16_t i = 0;
volatile uint32_t phase_accumulator = 0;

const uint8_t sineLookupTable[] PROGMEM = {
...
/*
    the use of overflow interrupt ensures precise time base
    overflow flag is set when counter reaches BOTTOM
    f_waveform = f_sampling * (phase_offset/2^phase_accumulator_width)
*/
ISR(TIMER0_OVF_vect) {
    phase_accumulator += PHASE_OFFSET;              // increment phase accumulator
    i = phase_accumulator >> 22;                    // upper 10 bits are used as address to lookup table
    OCR0A = pgm_read_byte(&sineLookupTable[i]);     // write amplitude value as PWM duty cycle
}